...
Memos are a type of reactive value that can be used to store computation results in cache, or memoize, derived state or expensive computations.
They are similar to [derived signals](/concepts/derived-values/derived-signals) but the reactive values automatically re-evaluate when their dependencies change.
Unlike derived signals, however, memos are optimized to execute only once for each change in their dependencies.
...
- They are optimized to execute only once for each change in their dependencies.
...
A pure function is one that does not cause any side effects, meaning the function's output should only depend on its inputs.
...
This can be avoided by using a [`createEffect`](/reference/createEffect) to handle the side effects instead:
