...
Refs, or references, are special attributes that can be attached to any element, and used to reference a DOM element or a component instance.
They are useful when direct access to DOM nodes is needed or invoking methods on a component.
...
This can be done by using [`onMount`](/reference/lifecycle/on-mount), which waits until the element is attached to the DOM before accessing it.

**Note:** DOM elements through element selectors is not recommended.
As elements with the same selectors are added and removed from the DOM, the first element that matches the selector will be returned, which may not be the desired element.

## JSX as a value

When trying access the DOM element, JSX can be used as a value and assigned to a variable.
...
It provides direct access to the element without having to wait until it is attached to the DOM.
This method can be used multiple times without having to worry about duplicate selectors.

The downside to this approach is that it separates the element and any child elements that are created from it from the rest of the DOM.
This removes the ability to access the element directly from the variable.
Similarly, reading through components that use JSX as a value can be more difficult to understand.

## Refs in Solid

Solid provides a ref system that gives access to the DOM element directly without having to wait until it is attached to the DOM.
These assignments occur at _creation time_ prior to the element being added to the DOM.
This gives access directly to the elements for manipulation, such as animations and event handling.

To use `ref`, declare a variable and use it as the `ref` attribute:
...
<Callout>
	In TypeScript,a definitive assignment assertion must be used. 
	Since Solid takes care of assigning the variable when the component is rendered, this signals to TypeScript that the variable will be assigned, even if it can't confirm it.
...
This helpful for when the element may not exist when the component is first rendered, or may be removed from the DOM at some point.
...
To see a detailed example of how refs are created and used, visit this [Solid playground example](https://playground.solidjs.com/anonymous/22a1abfa-a0f5-44a6-bbe6-40387cf63b95).
...
Once a child component receives the `ref`, it can be assigned to the element that the child component wants to expose through the `ref` attribute.
...
Directives are like callback refs but they provide two extra features:
...
Directive functions are called at render time, but before before the element has been added to the DOM.
...
Within directives, a variety of tasks can be performed, including:
