...
Context provides a way to pass data through a component tree without having to pass props down manually at every level.

## When to use context

With a large [component tree](/concepts/components/basics#component-trees) that requires state to be shared, context can be used.
Context can be used to avoid [prop drilling](/concepts/components/props#prop-drilling), which is the practice of passing props through intermediate elements without using them directly.

To avoid passing props through only a few levels, component composition is often a simpler solution than context.
[Signals](/concepts/signals) are often the simplest solution since they can be imported directly into the components that needs them.

Context is designed to share information that is regularly accessed by many components in your application or component tree.
...
const Provider = (props) => (
	<MyContext.Provider value="new value"> {/* `"new value"` overrides the initial value */}
		{props.children}
	</MyContext.Provider>
)
```
...
When passing multiple values into the `Provider`, you can destructure the context object to access the values needed.
This provides a readable and reactive way to access the values you need without having to access the entire context object:
...
To solve this issue, custom utilities can be created for a more readable way to access the context values.

For example, when wrapping a component tree, a custom `Provider` component can be created that can be used to wrap the component tree.
This also provides the option of re-using the `Provider` component in other parts of your application, if needed.
...
Now if you had to access the Provider in different areas of an application, you can import the `CounterProvider` component and wrap the component tree:
...
Instead of importing `useContext` and passing in the context object on each component it is being used in, creating a customized utility can make it easier to access the values needed:
...
When working with TypeScript, this can introduce type issues that make it difficult to determine why a component is not rendering as expected.
