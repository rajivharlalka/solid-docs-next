...
They read-only properties passed to components as attributes within JSX and are accessible within the component via the `props` object:
...
Since components in Solid do not own state, props are not needed to pass state between components, but they can be used.
