...
This sets up the effect and initializes variables or subscribes to any [signals](/concepts/signals) within it.
After this run, the effect will only be triggered when any of these _dependencies_ change.
...
This allows each each effect to independently track its own dependencies, without affecting the effect that it is nested within.
...
The order of execution is important when using effects.
An inner effect will _not_ affect the outer one.
Signals that are accessed within the inner effect, will _not_ be registered as dependencies for the outer effect.
Meaning, when the signal located within the inner effect changes, it will trigger only the _inner effect_ to re-run, not the outer one.
...
These functions provide a way to control the initialization and disposal of effects to build the type of behaviour that you need.
...
This is similar to an effect, but it does not track any dependencies.
...
When using an effect, there is no guarantee that it will only run once, which can lead to an undesired behaviour.
This makes `onMount` useful for API calls and other side effects where they only need to run once per component instance.

### `onCleanup`

While `onMount` is useful for running a side effect once, [`onCleanup`](/reference/onCleanup) is helpful for cleaning up a task when it is no longer needed.
...
To avoid the interval from running indefinitely, the `onCleanup` function in this example is used to clear the interval once the component unmounts.

`onCleanup` can also be used to avoid memory leaks.
