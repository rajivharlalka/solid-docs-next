...
This function takes an optional initial value as an argument, and returns a pair of functions: a **getter** function, and a **setter** function.
...
When a signal is called within a [tracking scope](/concepts/intro-to-reactivity#tracking), the signal adds the dependency to a list of subscribers.
...
<Callout>
	A tracking scope can be created by [`createEffect`](/reference/createEffect) or [`createMemo`](/reference/createMemo), which are other Solid primitives.
