...
List rendering provides a way to generate multiple elements from a collection of data, such as an array or object, where each element corresponds to an item in the collection.
...
The sole property in `<For>` is `each` , where the data collection to loop over is passed.
...
Between the `<For>` tags, the component requires a [callback function](https://developer.mozilla.org/en-US/docs/Glossary/Callback_function) which will dictate how each item should be rendered.
...
- `item`: the current item in the data collection that is being rendered over.
...
Similar to the `<For>` component, `<Index>` accepts a single property named `each` where you pass the structure you wish to loop over.
