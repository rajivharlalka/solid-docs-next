...
Using `<Portal>` can be useful in cases where elements, like information popups, might be clipped or hidden due to the overflow settings of their parent elements.
By putting the element outside of the parent element, it is no longer bound by the overflow settings of its parent.
This provides a more accessible experience for users since content is no longer obscured.