...
Similar to [signals](/essentials/signals), stores are state management primitive.
However, while signals manage a single piece of state, stores create a centralized location to reduce code redundancy.
Within Solid, these stores can create a collection of signals, each corresponding to a particular property which can be useful when working with complex state.

## Creating a store

Stores were designed to manage data structures like objects and arrays but can contain other data types, such as strings and numbers.
...
Updating values within a store is best accomplished using the setter provided by the `createStore` initialization.
...
These signals are created lazily, meaning they are once accessed within a reactive context, such as in a template or a computed property, signals are created and dependencies are established.

For example, to update `store.userCount` with every new user, writing `setStore("userCount", store.userCount.length)` would not work.
Rather, to add this in a tracking scope, such as within a [`createEffect`](/reference/basic-reactivity/create-effect) so that a dependency is established.
...
Modifying a store using the above method is referred to as "path syntax."
...
By indicating these exact key names, you can directly retrieve the targeted information.
...
This method grants the ability to select multiple properties within the store, providing access to nested structures.
Alternatively, filtering functions can be used to access keys based on dynamic conditions or specific rules.
...
The flexibility in path syntax makes for efficient navigation, retrieval, and modification of data in a store, regardless of the store's complexity or the requirement for dynamic access scenarios within the application.
...
For example, to append the new element to the end of the array, set the index to `array.length`:
...
With path syntax, a range of indices to update or modify can be used to target a subset of the elements within the array for modification.
...
These functions receive the old value as an argument, providing a way to compute the new value based on the existing one.
...
Where you want to update elements in an array based on a specific condition, you can pass a function as an argument.
This function will act as a filter to select elements that satisfy the condition.
...
In addition to [`.startsWith`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/startsWith), other array methods like [`.find`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/find) can be used to filter for the values needed.

## Modifying objects

When using store setters to modify objects, if a new value is an object, it will be shallow merged with the existing value.
What this means is properties of the existing object will be combined with the properties of the "new" object, where updates will happen with any overlapping properties with the values from the new object.

This directly makes the change to the store _without_ spreading out properties of the existing user object.
...
`produce` also gives a way to make changes to multiple properties at the same time which eliminates the need for multiple setter calls.
...
`produce` creates a temporary draft of the state, applies the changes, then produces a new [immutable](https://developer.mozilla.org/en-US/docs/Glossary/Immutable) version of the store.
Comparatively, `setStore` provides a more straightforward way to update the store directly, without creating a new version.

It is important to note, however, `produce` is specifically designed to work with **arrays** and **objects**.
...
Firstly, it provides a snapshot of the current state without the processing overhead associated with reactivity in situations where an unaltered, non-reactive view of the data is needed.
...
This utility acts as a bridge to facilitate smooth integrations with external components and simplifies adding stores into various applications and workflows.
