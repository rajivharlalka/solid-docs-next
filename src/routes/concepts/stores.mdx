...
While signals are created using the [`createSignal`](/reference/createSignal) function and require calling the signal function to access their values, store values can be directly accessed without a function call.
...
Rather, you would have to add this in a tracking scope, such as within a [`createEffect`](/reference/createEffect) so that a dependency is established.
