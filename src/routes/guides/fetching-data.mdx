...
The fetcher function that is created makes a call to get the data, which is then passed in as an argument to `createResource`. 
...
* `state`: The current status of the operation (`unresolved`, `pending`, `ready`, `refreshing`, or `error`).
...
[`Switch/Match`](/concepts/control-flow/conditional-rendering) provides one way to manage these conditions. 
...
[`Suspense`](/reference/components/suspense) is a component in Solid designed to act as a boundary. 
...
Additionally, you can nest as many components as needed within `Suspense`, only the closest ancestor will switch to the `fallback` state when a loading state is detected.
