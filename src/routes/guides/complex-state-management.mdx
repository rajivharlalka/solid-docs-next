...
If you are new to the concept of stores, see the [stores section](/concepts/stores).

## Creating a store

To reduce the amount of signals that were used in the original example, the following can be done using a store:
...
Through using a store, there is no longer a need to keep track of separate signals for `tasks`, `numberOfTasks`, and `completedTasks`.
...
Through `state.numberOfTasks`, the display will show the store's value held in the `numberOfTasks` property.
...
// without produce
batch(() => {
	setState(0, "text", "I am updated text")
...
setState(
	0,
	produce((task) => {
		;(task.text = "I am updated text"), (task.completed = true)
	})
)
```
