...
To create a piece of state, you use the [`createSignal`](/reference/createSignal) function and pass in the initial value of the state:
...
The UI is the visual representation of the state to the user, and is [rendered using JSX](/concepts/understanding-jsx).
...
This is known as [fine-grained reactivity](/advanced/fine-grained-reactivity).
...
The [`createEffect`](/reference/createEffect) function sets up a function to perform side effects whenever the state is modified.
...
Derived values can be created through using a signal within a function, which can be referred to as a [derived signal](/derived-values/derived-signals):
...
While this approach works, it can be inefficient as the `squaredCount` signal will be re-evaluated on every render.
...
Using the [`createMemo`](/reference/createMemo) function, you can create a memoized value that is only re-evaluated when its dependencies change.
...
This can keep things synchronized across the [component tree](/guides/components-and-props#component-trees) and allow for more predictable state management.
