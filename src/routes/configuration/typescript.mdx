...
The API Documentation details the types of API calls.
...
Importantly, calling `setCount` without any arguments will reset the signal's value to `undefined`.
...
For example, when using `useContext` with the context type `Context<Data | undefined>`, the returned result will be of the type `Data | undefined`.
...
<Callout>
	...
	[`createMemo`](/reference/createMemo),
	[`createRenderEffect`](/reference/createRenderEffect), and
	[`createComputed`](/reference/createComputed).
</Callout>
...
It is worth noting that runtime type errors may arise from doing this.
...
In Solid, the [`prop:___`](/reference/jsx-attributes/prop) directive allows explicit property setting, which is useful for retaining the original data types like objects or arrays.
[`attr:___`](/reference/jsx-attributes/attr) directive allows custom attributes, on the other hand, and it is effective for handling string-based HTML attributes.
